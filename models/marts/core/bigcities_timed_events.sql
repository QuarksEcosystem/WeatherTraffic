...
big_cities_hours_traffic as(
    select big_cities_days.city,  to_timestamp(concat(days,' ', hours), 'dd/mm/yyyy hh24:mi:ss') as event_timestamp, listagg(distinct type, ', ') within group (order by type) as traffic_events from big_cities_days
